...
col1, col2 = st.columns([3, 1])  # Left (Title) | Right (Calendar)

with col1:
    st.image("https://vtlogo.com/wp-content/uploads/2020/03/k-electric-vector-logo.png", width=120)

with col2:
    # 🔹 FIXED: Added a label & used `label_visibility="hidden"`
    selected_date = st.date_input("Select Date", datetime.date.today(), label_visibility="hidden")



# ✅ Display Titles

st.markdown("<h4 style='text-align: center; color:#3498db;'>📋 DUTY ROASTER CM GIZRI \n\n (30 MARCH TO 3rd may)</h4>", unsafe_allow_html=True)
...
# ✅ If "All Teams Details" is selected, show all team details in a list format
if view_option == "All Teams Details":
    ...
    # ✅ Display all team members in a list format
